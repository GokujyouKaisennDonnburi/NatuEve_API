...
4. `profile` を足すと既存の `profileId` と情報が重複するが、`profileId` を残すか削るか。
...
追加する。匿名参加（`profile_id` が NULL）は `profile: null` を返す（ポインタ型）。`profile.id` と
完全に重複する `profileId` は同時に削除する（破壊的変更）。**
...
4. `profileId` を削除する。FK により `profileId` と `profile.id` は常に一致し、両方 null か両方非 null に
   しかならないため、`profileId` の情報は `profile` から完全に復元できる。匿名参加の判定は
   `profile === null` で行う。
...
- なお FK があるため、`profile_id` が非 NULL なら `profiles` の行は必ず存在する。よって
  `profile` が `null` になるのは匿名参加のときだけで、「ログイン済みなのに `profile` が引けない」状態は
  起こらない。この保証があるため、匿名判定を `profile === null` の一本に寄せられる。

### 重複する `profileId` を削除する

FK の保証により `profileId` と `profile.id` は常に一致し、両方 null か両方非 null にしかならない。
つまり `profileId` の情報は `profile` から 100% 復元でき、独立した価値を持たない。

当初は互換性を優先して残す判断をしたが、「`profile` の中を覗かずに匿名判定できる」という利点は
`profile === null` と書く手間と変わらず、実質的な価値が無いと結論した。残る理由は既存クライアントへの
配慮だけで、それはフィールドの価値ではなく削除のコストにすぎない。冗長なフィールドを残すと、
クライアントはどちらを正とするか判断を迫られ、両者がズレ得るという誤解も生む。

イベント詳細（`EventResponse`）は既に `profile` のみで `profileId` を持っておらず、削除する方がこの
リポジトリの既定路線に沿う。イベント一覧（`EventSummary`）も同じ重複を抱えているが、認証不要の公開
エンドポイントで影響範囲が広いため本 ADR の対象外とし、別 issue で扱う。
...
- 冗長な `profileId` が消え、参加者のアカウント参照が `profile` 一箇所に集約される。`EventResponse` と
  同じ形になり、エンドポイント間の表現が 1 つ揃う。
...
- **破壊的変更である**。`profileId` を参照している既存クライアントは `profile.id` へ、匿名判定は
  `profileId === null` から `profile === null` へ書き換えが要る。影響が主催者向けの 1 エンドポイントに
  限られることを前提に、移行の手間より重複解消を優先した。公開エンドポイントの `EventSummary` を
  同時に直さなかったのはこの前提が成り立たないため。
- **null 表明はベンダー拡張に依存する**。`x-nullable` は Swagger 2.0 の拡張であり、クライアント側の
  コード生成器がこれを解釈するかに依存する。解釈しない生成器では型で null 処理を強制できず、
  `@Description` の記述が頼りになる。
...
- **`omitempty` を付けて匿名時はキーごと省略する**: 不採用。理由は 3 つある。
  ...
  2 つ目は**匿名判定の唯一の手掛かりを弱めてしまう**こと。`profileId` を削除した結果、匿名参加かどうかは
  `profile` が null かで判定する。キーごと消すと、この判定が「キーの不在」という最も弱い形でしか
  表現されなくなる。
...
- null 表明: `docs: 参加者一覧の profileId に x-nullable を付与 #162`（後に profileId ごと削除）
- 欠損表現の基準: `docs: レスポンスの欠損表現の基準を追記 #162`
- 名前の使い分け: `docs: username と displayName の違いを明記 #162`
- 重複の解消: `update: 参加者一覧から profileId を削除 #162`
