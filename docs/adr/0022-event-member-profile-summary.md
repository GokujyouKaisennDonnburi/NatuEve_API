...
- **`omitempty` を付けて匿名時はキーごと省略する**: ペイロードは軽くなるが、「キーが無い」は
  「まだ実装されていない」とも読めて曖昧になる。ADR-0015 が一覧の `tags` で `omitempty` を選んだのは
  要素数が多く空配列の重複が効くケースで、単一の nullable オブジェクトには当てはまらない。明示的な
  `null` の方が意味が一意に定まるため不採用。
