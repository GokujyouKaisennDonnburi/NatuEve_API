...
const docTemplate = `{
    ...
    "paths": {
            ...
            "get": {
                ...
                "parameters": [
                    ...
                    },
                    {
                        ...
                        "collectionFormat": "multi",
                        "description": "絞り込む地域(location への部分一致。反復指定でOR検索。最大200件・1件255文字以内)",
                        "name": "location",
                        "in": "query"
                    },
                    {
        ...
        "/api/v1/events/{id}/absence": {
            "post": {
                ...
                "description": "認証必須。ログイン参加者が欠席理由を添えて参加を取り消す。\n参加行を削除し、参加状態ログへ action=absence を1件追記する。\n主催者宛ての欠席連絡メールを outbox に予約し、非同期で送信する（レスポンスには含まれない）。\nreason は任意。指定する場合は illness(体調不良) / family(家庭の都合) /\nweather_transport(天候・交通) / other(その他) の4値のいずれか。\nreason 未指定時はレスポンスの reason が null になり、主催者メールには「記載なし」と記す。\ndetail は任意（trim 後200文字以内）。未指定時はレスポンスの detail が null になる。\n申込期限ありイベントで期限前に呼ぶと 409 Conflict（before_deadline）を返す\n（期限前のキャンセルは参加キャンセル API を利用する）。\nend_date 経過後は 409 Conflict（event_ended）、取消済みイベントは 409 Conflict（event_cancelled）を返す。\nそのイベントに参加していない場合は 404 not_found を返す。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                ...
                "parameters": [
                    ...
                    },
                    {
                        ...
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_GokujyouKaisennDonnburi_NatuEve_API_internal_model.AbsenceEventRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_GokujyouKaisennDonnburi_NatuEve_API_internal_model.AbsenceEventResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                    ...
                    "409": {
                        "description": "before_deadline: 申込期限前 / event_ended: 終了済み / event_cancelled: 取消済み",
                        "schema": {
                            "$ref": "#/definitions/github_com_GokujyouKaisennDonnburi_NatuEve_API_internal_model.AbsenceConflictErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
        ...
        "/api/v1/events/{id}/cancel": {
            "post": {
                ...
                "description": "イベント主催者がイベントを開催取りやめにする。主催者のみ実行可能。\n非冪等: 参加者へ送る通知メールの件名・本文は任意で受け取り、省略時は\nサーバーが既定文面を補ったうえで、キャンセル確定と同一トランザクションで\n通知を outbox に予約する（バックグラウンドワーカーが個別送信する）。\n既にキャンセル済みのイベントに対する呼び出しは 409 を返す。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
        ...
        "/api/v1/events/{id}/join": {
            "post": {
                ...
                "responses": {
                    ...
                    "409": {
                        "description": "already_joined: 既に参加しています / capacity_full: 定員に達しています / deadline_passed: 申込期限経過後 / event_ended: イベント終了後",
                        "schema": {
        ...
        "/api/v1/events/{id}/leave": {
            "post": {
                ...
                "description": "認証必須。ログイン参加者が自身の参加を取り消す。\n参加行を削除し、参加状態ログへ action=leave を1件追記する。\n匿名参加（profileId=null）は本 API の対象外。\n申込期限ありイベントで期限経過後の呼び出しは 409 Conflict（deadline_passed）で拒否される。\n期限経過後のキャンセルは欠席連絡 API（POST /api/v1/events/{id}/absence）を利用する。\nそのイベントに参加していない場合は 404 not_found を返す。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                ...
                "responses": {
                    ...
                    "409": {
                        "description": "deadline_passed: 申込期限経過後",
                        "schema": {
                            "$ref": "#/definitions/github_com_GokujyouKaisennDonnburi_NatuEve_API_internal_model.LeaveConflictErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
            ...
            "get": {
                ...
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_GokujyouKaisennDonnburi_NatuEve_API_internal_model.MyEventApplicationResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                    ...
                    "404": {
                        "description": "not_found: イベント不存在 または 未申込",
                        "schema": {
                            "$ref": "#/definitions/github_com_GokujyouKaisennDonnburi_NatuEve_API_internal_model.NotFoundErrorResponse"
                        }
                    },
        ...
        "/api/v1/profiles/{id}/events": {
            "get": {
                "description": "指定したユーザーが主催したイベント／参加済みイベントを返す。認証不要。\ntype は必須。\"hosted\"(主催したイベント) / \"attended\"(参加済みイベント) のいずれかで、\n未指定・不正値は 400 を返す。\"applied\"(申し込み中) は本人限定のため公開せず、\n本人が取得する場合は GET /api/v1/me/events を使う。\nhosted はそのユーザーが投稿したイベント全件（過去・キャンセル済みを含む）。\nattended は申込済みでイベント終了日時(endDate)を過ぎたもの。\n参加をキャンセル(leave)したイベントと、ログインせずに申し込んだイベントは attended に含まれない。\ncounts には公開する2種別の件数を常に含める（タブのバッジ表示用）。totalCount は type に対応する件数。\nevents の各要素は GET /api/v1/events と同じ EventSummary。\nsort は \"created_at\"(デフォルト) / \"event_date\" のみ許可。不正値はデフォルトに戻す。\norder は \"desc\"(デフォルト) / \"asc\" のみ許可。不正値はデフォルトに戻す。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                ...
                "parameters": [
                    ...
                    {
                        "type": "string",
                        "description": "取得する種別(hosted|attended)",
                        "name": "type",
                        "in": "query",
                        "required": true
                    },
                ...
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_GokujyouKaisennDonnburi_NatuEve_API_internal_model.ProfileEventListResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
    ...
    "definitions": {
        "github_com_GokujyouKaisennDonnburi_NatuEve_API_internal_model.AbsenceConflictErrorBody": {
            "type": "object",
            "properties": {
                "code": {
                    ...
                    "example": "before_deadline"
                },
                "message": {
                    "description": "Message は人間向けのエラーメッセージ。",
                    "type": "string",
                    "example": "申込期限前に欠席連絡はできません。参加キャンセル API を利用してください"
                }
            }
        },
        "github_com_GokujyouKaisennDonnburi_NatuEve_API_internal_model.AbsenceConflictErrorResponse": {
            "type": "object",
            "properties": {
                "error": {
                    "$ref": "#/definitions/github_com_GokujyouKaisennDonnburi_NatuEve_API_internal_model.AbsenceConflictErrorBody"
                }
            }
        },
        "github_com_GokujyouKaisennDonnburi_NatuEve_API_internal_model.AbsenceEventRequest": {
            "description": "申込期限経過後の参加キャンセルに必要な情報。認証必須。 reason は任意（指定する場合は4値のいずれか）。detail は任意（trim 後200文字以内）。",
            "type": "object",
            "properties": {
                "detail": {
                    "description": "Detail は欠席理由の詳細（任意・200文字以内）。",
                    "type": "string",
                    "example": "熱が出たため"
                },
                "reason": {
                    "description": "Reason は欠席理由（任意）。illness=体調不良 / family=家庭の都合 /\nweather_transport=天候・交通 / other=その他。\nフィールド省略・null・空文字はいずれも未指定として扱う。",
                    "allOf": [
                        {
                            "$ref": "#/definitions/github_com_GokujyouKaisennDonnburi_NatuEve_API_internal_model.AbsenceReason"
                        }
                    ],
                    "example": "illness"
                }
        ...
        "github_com_GokujyouKaisennDonnburi_NatuEve_API_internal_model.AbsenceEventResponse": {
            "description": "欠席連絡の結果。追記された参加状態ログ（action=absence）1件分の内容を返す。 reason・detail は未指定で欠席連絡した場合 null。 主催者宛ての欠席連絡メールは outbox に予約され、非同期で送信される（レスポンスには含まれない）。",
            "type": "object",
            "properties": {
                "action": {
                    "description": "Action は参加状態ログのアクション。常に \"absence\"。",
                    "type": "string",
                    "example": "absence"
                },
                "createdAt": {
                    "description": "CreatedAt は参加状態ログ（absence）の記録日時。",
                    "type": "string",
                    "example": "2026-09-02T12:00:00Z"
                },
                "detail": {
                    "description": "Detail は記録された欠席理由の詳細。未指定時は null。",
                    "type": "string",
                    "x-nullable": true,
                    "example": "熱が出たため"
                },
                "eventId": {
                    "description": "EventID は欠席連絡したイベントのUUID。",
                    "type": "string",
                    "example": "a1b2c3d4-e5f6-7890-abcd-ef1234567890"
                },
                "profileId": {
                    "description": "ProfileID は欠席連絡したユーザーのUUID。absence は認証必須のため常に値が入る。",
                    "type": "string",
                    "example": "b2c3d4e5-f6a7-8901-bcde-f23456789012"
                },
                "reason": {
                    "description": "Reason は記録された欠席理由。未指定時は null。",
                    "type": "string",
                    "x-nullable": true,
                    "example": "illness"
                }
        ...
        "github_com_GokujyouKaisennDonnburi_NatuEve_API_internal_model.AbsenceReason": {
            "type": "string",
            "enum": [
                "illness",
                "family",
            ...
            "x-enum-varnames": [
                "AbsenceReasonIllness",
                "AbsenceReasonFamily",
                "AbsenceReasonWeatherTransport",
                "AbsenceReasonOther"
            ]
        },
        "github_com_GokujyouKaisennDonnburi_NatuEve_API_internal_model.CancelEventRequest": {
            "description": "イベント取りやめ確定時に参加者へ送る通知の件名・本文。同一トランザクションで outbox に予約され、バックグラウンドワーカーが参加者へ個別送信する。 件名・本文はいずれも任意で、省略（空文字）時はサーバーが既定文面を補って予約する。",
            "type": "object",
            "properties": {
                "body": {
                    "description": "Body は参加者へ送る通知メールの本文（任意・10,000文字以内）。\n省略（空文字）時はサーバーが既定文面を補う。",
                    "type": "string",
                    "maxLength": 10000,
                    "example": "台風接近に伴い、安全のため本イベントは中止とさせていただきます。"
                },
                "subject": {
                    "description": "Subject は参加者へ送る通知メールの件名（任意・255文字以内）。\n省略（空文字）時はサーバーが既定文面を補う。",
                    "type": "string",
                    "maxLength": 255,
        ...
        "github_com_GokujyouKaisennDonnburi_NatuEve_API_internal_model.CreateEventRequest": {
            ...
            "properties": {
                "applicationDeadline": {
                    "description": "ApplicationDeadline は申込期限(RFC3339)（任意）。省略時は期限なし。終了日時以前であること。",
                    "type": "string",
        ...
        "github_com_GokujyouKaisennDonnburi_NatuEve_API_internal_model.EventListResponse": {
            ...
            "properties": {
                ...
                "totalCount": {
                    "description": "TotalCount は現在の絞り込み条件（q / tagId / status / location）に一致する総件数。\n条件を指定しない場合は全件数になる。クライアントが最終ページ offset を算出するために使う。",
                    "type": "integer",
                    "example": 153
                }
            }
        ...
        "github_com_GokujyouKaisennDonnburi_NatuEve_API_internal_model.LeaveConflictErrorBody": {
            "type": "object",
            "properties": {
                "code": {
                    "description": "Code は競合の原因を表す機械可読なエラーコード。deadline_passed = 申込期限経過後。",
                    "type": "string",
                    "enum": [
                        "deadline_passed"
                    ],
                    "example": "deadline_passed"
                },
                "message": {
                    "description": "Message は人間向けのエラーメッセージ。",
                    "type": "string",
                    "example": "申込期限経過後の参加キャンセルは欠席連絡 API を利用してください"
                }
            }
        },
        "github_com_GokujyouKaisennDonnburi_NatuEve_API_internal_model.LeaveConflictErrorResponse": {
            "type": "object",
            "properties": {
                "error": {
                    "$ref": "#/definitions/github_com_GokujyouKaisennDonnburi_NatuEve_API_internal_model.LeaveConflictErrorBody"
                }
            }
        },
        ...
        "github_com_GokujyouKaisennDonnburi_NatuEve_API_internal_model.MyEventApplicationResponse": {
            "type": "object",
            "properties": {
                "createdAt": {
                    "description": "CreatedAt は参加申込日時。",
                ...
                "eventId": {
                    "description": "EventID は申込先イベントのUUID。",
                    "type": "string",
                    "example": "a1b2c3d4-e5f6-7890-abcd-ef1234567890"
                },
                "mailAddress": {
                    "description": "MailAddress は申込時に入力されたメールアドレス。",
                    "type": "string",
                    "example": "yamada@example.com"
                },
                "participants": {
                ...
                "username": {
                    "description": "Username は申込時にフォームへ入力された名前。",
                    "type": "string",
                    "example": "山田太郎"
                }
            }
        ...
        "github_com_GokujyouKaisennDonnburi_NatuEve_API_internal_model.ProfileEventCounts": {
            "type": "object",
            "properties": {
                "attended": {
                    "description": "Attended は参加済みのイベントの件数。",
        ...
        "github_com_GokujyouKaisennDonnburi_NatuEve_API_internal_model.ProfileEventListResponse": {
            "description": "指定種別のイベント一覧と、公開する2種別の件数。",
            "type": "object",
            "properties": {
                "counts": {
                    "description": "Counts は公開する2種別の件数。タブのバッジ表示に使う（1回のリクエストで揃う）。",
                    "allOf": [
                        {
                            "$ref": "#/definitions/github_com_GokujyouKaisennDonnburi_NatuEve_API_internal_model.ProfileEventCounts"
                        }
                    ]
                },
                "events": {
                    "description": "Events はイベントサマリーの一覧（リクエストの type に対応する）。",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_GokujyouKaisennDonnburi_NatuEve_API_internal_model.EventSummary"
                    }
